...
const FilterSection = () => {
  const [expendColor, setExpenColor] = useState(false);
  // const [expendBranch, setExpendBranch] = useState(false);
  const [searchParams, setSearchParams] = useSearchParams();


  const handleColorToggle = () => {
    setExpenColor(!expendColor);
  }
        ...
        <section>
          <FormControl>
            ...
            <RadioGroup aria-labelledby="color" defaultValue="" name="color" onChange={updateFilterParams}>
              {colors.slice(0, expendColor?colors.length:6).map((item) => (
                <FormControlLabel
                  value={item.name}
                  control={<Radio />}
                  label={
          ...
          <div>
            <button 
             ...
             className="text-primary-color cursor-pointer hover:text-teal-900 flex items-center">
              {expendColor?"hidden":`+ ${colors.length - 6} more`}
            </button>
          </div>
        </section>
        <Divider/>
